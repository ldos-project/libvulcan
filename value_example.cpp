...
int main() {
    ...
    // EVOLVE-BLOCK-START
    config1.add_listeners(rtt, {vulcan::listeners::global::RollingWindow(5), vulcan::listeners::global::MinMax()});
    config1.add_listeners(queue_len, {vulcan::listeners::global::RollingWindow(5), vulcan::listeners::global::MinMax()});
    ...
    config2.add_listeners(rtt, {vulcan::listeners::global::RollingPercentile(100)}); // num of objects = 100
    auto flow2_cwnd_fn = [&](const vulcan::feature_store& fs) -> double {
      return fs.get_percentile(rtt, 95); // p95
    };
    ...
    // Use deque to avoid reference invalidation since value_policy (and its feature_store) are non-copyable.
    std::deque<vulcan::value_policy> policies;
    std::vector<std::reference_wrapper<vulcan::feature_store>> stores;
    
    for(int i=0; i<10; i++) {
        if(i%2 == 0) policies.push_back(vulcan::instantiate_value_policy(registry, config1));
        else policies.push_back(vulcan::instantiate_value_policy(registry, config2));
        stores.push_back(policies.back().get_feature_store());
    }

    std::cout << "\n" << policies.front().get_prompt() << std::endl;
    
    std::srand(static_cast<unsigned>(std::time(nullptr)));
    for(int t=0; t<10; t++) { // timestep 
        for(int j=0; j<2; j++) { // policy number
            auto curr_config = (j%2 == 0) ? config1 : config2;
            if(curr_config.has_listeners(rtt)) stores[j].get().update(rtt, 50.0 + (std::rand() % 100));
            if(curr_config.has_listeners(queue_len)) stores[j].get().update(queue_len, static_cast<int64_t>(std::rand() % 20));
            double cwnd = vulcan::decision(policies[j]);
            if(curr_config.has_listeners(prev_cwnd)) stores[j].get().update(prev_cwnd, cwnd);
